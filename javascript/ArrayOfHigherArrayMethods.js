...
const empIDs = employees.filter( emp => emp.designation == 'IT' || employeeNames.designation =='HR'  && emp.end - emp.start >=6).map(emp => emp.id)
...
console.log(ages);
// first value - second value - prints in accending order
// second value - first value - prints in accending order
...
const sum = demoArr.reduce( (number , initialsum) => (number+initialsum) , 0 )
console.log(sum);